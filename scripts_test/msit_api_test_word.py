...
from msit_gen_word import gen_msit_rep_sequence, msit_instruciton, as_prompt_line
...
def run_single_session(session_id: int, ndigits: int, stim_types_str: str, nrep: int,
                      restriction: str, model_name: str, max_tokens: int, formality: str,
    ...
    try:
        if exhaustive_stimulus is not None:
            ...
            if add_head_tail_space:
                stimuli = " " + stimuli + " "
            answers = [exhaustive_stimulus.target_pos_index]  # 1-based position
    ...
    instruction = msit_instruciton(ndigits, by_image=False, restriction=restriction, 
                                  ...
                                  formality=formality,nstims=len(answers))
...
def main():
    ...
    if args.exhaustive:
        ...
        from msit_gen_word import generate_exhastive_stims_by_type
        
        # Get the single condition type
        condition = int(args.stim_types.split(',')[0].strip())
        
        # Generate all possible stimuli for this condition
        exhaustive_stimuli = generate_exhastive_stims_by_type(args.ndigits, cond=condition, formality=args.formality)
